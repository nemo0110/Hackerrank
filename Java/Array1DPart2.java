/*
Standing at index 0 of an n element array named Game.
...
The canWin function has the following parameters:
...
The second line contains n space-separated binary integers (i.e., zeroes and ones) describing the respective values of game[0],...,game[n-1].
...
2.  For game[0,0,0,1,1,1] and leap = 5, we can walk to index 1 and then jump i + leap = 1+5 = 6 units to the end of the array.
    Because we can win, we return true.
3.  For For game[0,0,1,1,1,0] and leap = 3, there is no way for us to get past the three consecutive ones.
